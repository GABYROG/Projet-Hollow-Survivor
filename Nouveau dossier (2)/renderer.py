...
def _draw_swing_effects(surf, gs, cam_x, cam_y):
    ...
    for wid in player.weapons:
        w = WEAPONS.get(wid)
        if not w or w["type"] not in ("melee_swing","whip","melee_area"): continue
...
def _draw_enemy(surf, enemy, cam_x, cam_y, font_tiny):
    ...
    if isinstance(enemy, BlueSlime):
        if not (-128 <= ex <= WIDTH+128 and -128 <= ey <= HEIGHT+128): return
        frame = enemy.current_frame()
        if frame:
            offset = (128 - enemy.SIZE) // 2
            surf.blit(frame, (ex - offset, ey - offset))
        return
...
def _draw_projectiles(surf, projectiles, cam_x, cam_y):
    for proj in projectiles:
        px=int(proj.x-cam_x); py=int(proj.y-cam_y)
        r=proj.r
        if not(-r-2<=px<=WIDTH+r+2 and -r-2<=py<=HEIGHT+r+2): continue
        # Glow
        glow=pygame.Surface((r*4+4,r*4+4),pygame.SRCALPHA)
        pygame.draw.circle(glow,(*proj.color,50),(r*2+2,r*2+2),r*2)
        pygame.draw.circle(glow,(*proj.color,200),(r*2+2,r*2+2),r)
        pygame.draw.circle(glow,(255,255,255,230),(r*2+2,r*2+2),max(1,r//2))
        surf.blit(glow,(px-r*2-2,py-r*2-2))
        # Trainee
        angle=proj.angle
        for t in range(1,6):
            tx=int(px-math.cos(angle)*t*r*0.8)
            ty=int(py-math.sin(angle)*t*r*0.8)
            tr_s=pygame.Surface((r*2,r*2),pygame.SRCALPHA)
            alpha=int(120*(1-t/6))
            pygame.draw.circle(tr_s,(*proj.color,alpha),(r,r),max(1,r-t))
            surf.blit(tr_s,(tx-r,ty-r))
...
def draw_weapon_icon(surf, wid, cx, cy, col):
    if wid=="epee":
        ...
        pygame.draw.arc(surf,col,(cx-10,cy-12,20,24),math.radians(60),math.radians(300),3)
        pygame.draw.line(surf,(180,140,80),(cx-5,cy-10),(cx-5,cy+10),1)
        pygame.draw.line(surf,col,(cx-5,cy),(cx+8,cy),2)
    elif wid=="baguette":
