"""
wave_manager.py — Gestion du temps de partie, des vagues et des boss.

Logique :
  - Spawn d'ennemis en continu base sur le temps ecoule
  - 0-10s  : seulement des BasicEnemy, 1/s
  - 10-30s : BasicEnemy + FastEnemy, 1/s
  - 30-60s : + TankEnemy, 1.5/s
  - 60s+   : + RangedEnemy, montee progressive jusqu a 3/s
  - Toutes les 2 minutes : 1 boss (5 boss au total)
  - A 10 minutes : victoire si les 5 boss sont vaincus
"""

import random
import pygame
from entities import BasicEnemy, FastEnemy, TankEnemy, RangedEnemy
from bosses   import BOSS_SEQUENCE

# Paliers de temps (secondes) -> (types, ennemis_par_seconde)
PALIERS = [
    (  0, [BasicEnemy],                                        1.0),
    ( 10, [BasicEnemy, FastEnemy],                             1.0),
    ( 30, [BasicEnemy, FastEnemy, TankEnemy],                  1.5),
    ( 60, [BasicEnemy, FastEnemy, TankEnemy, RangedEnemy],     2.0),
    (120, [BasicEnemy, FastEnemy, TankEnemy, RangedEnemy],     2.5),
    (240, [BasicEnemy, FastEnemy, TankEnemy, RangedEnemy],     3.0),
]
...
class WaveManager:
    ...
    def update(self, gs, now, speed_f=1.0):
        ...
        if len(gs.enemies) < MAX_ENEMIES:
            # Le spawn ralentit aussi en slowmo
